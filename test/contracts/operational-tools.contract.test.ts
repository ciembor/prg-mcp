...
describe("operational MCP tools", () => {
  ...
  it("uses record-count fallback per layer when catalog coverage exists for a different layer", async () => {
    const dataDir = await mkdtemp(join(tmpdir(), "prg-layer-fallback-"));
    const { boundariesPath, catalogPath } = initializePrgDatabases({ addressShardCodes: ["14"], dataDir });
    const catalog = new Database(catalogPath);
    try {
      const snapshot = catalog.prepare(`
        insert into snapshots(dataset_key, scope, state_date, state_date_key, downloaded_at, checked_at, sha256, record_count, schema_fingerprint, adapter_version, source_url)
        values ('current:A00','country:PL',null,'','2026-06-23','2026-06-23','current',1,'schema','1','https://example.test')
      `).run();
      catalog.prepare(`
        insert into installed_coverage(layer_id,dataset_key,archive_year,scope_type,scope_code,snapshot_id,completeness)
        values ('A00','current:A00',0,'country','PL',?,'complete')
    ...
    try {
      boundaries.prepare(`
        insert into areas(rowid, snapshot_id, layer_id, object_id, name, normalized_name, min_x, min_y, max_x, max_y, geometry_wkb)
        values (1, 1, 'A03', 'local-only', 'Local only', 'local only', 0, 0, 1, 1, X'00')
      `).run();
    } finally {
      boundaries.close();
    }

    const layers = (await callTool(createApp(testConfig(dataDir)), "list_layers", {})).structuredContent as { layers: Array<{ layerId: string; available: boolean }> };
    expect(layers.layers.find((layer) => layer.layerId === "A03")).toMatchObject({ available: true });
  });
